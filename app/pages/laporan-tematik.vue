  ...
  <div class="min-h-screen bg-[#f7f9fc] text-slate-900">
    <header class="border-b border-slate-300 bg-white">
      <div class="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
        <div>
          <h1 class="text-2xl font-bold">Laporan Data Tematik Rak Buku</h1>
    ...
    <main class="mx-auto w-full max-w-7xl space-y-4 px-4 py-6 sm:px-6 lg:px-8">
      <section class="rounded-lg border border-slate-300 bg-white p-5 shadow-sm">
        <div class="grid gap-4 lg:grid-cols-[2fr,1fr]">
          <div>
            <h2 class="text-xl font-bold text-slate-900">Laporan Reservasi Data Tematik</h2>
...
const REPORT_TITLE = 'LAPORAN STOK TEMATIK CV MULTI BELIMA INDO'
const REPORT_SUBTITLE = 'LAPORAN DATA TEMATIK RAK BUKU'
...
const exportToExcel = () => {
  try {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Laporan Data Tematik')
    XLSX.utils.book_append_sheet(wb, buildMonthlySummarySheet(XLSX, dataList.value, 'DIAGRAM BULANAN STOK TEMATIK'), 'Diagram Bulanan')

    const fileName = `Laporan_Data_Tematik_${new Date().toISOString().slice(0, 10)}.xlsx`
...
const exportFilteredToExcel = () => {
  try {
    ...
    XLSX.utils.book_append_sheet(wb, buildMonthlySummarySheet(XLSX, filteredData.value, 'DIAGRAM BULANAN STOK TEMATIK FILTER'), 'Diagram Bulanan')

    const fileName = `Laporan_Data_Tematik_Filter_${new Date().toISOString().slice(0, 10)}.xlsx`
    downloadWorkbook(wb, fileName)
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error('Error fetching report tematik data:', error)
