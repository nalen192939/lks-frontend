  ...
  <div class="min-h-screen bg-slate-100 text-slate-900">
    <header class="border-b border-slate-200 bg-white">
      <div class="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-5 sm:px-6 lg:px-8">
        <div>
          <h1 class="text-2xl font-bold">Data Tematik</h1>
          <p class="mt-1 text-sm text-slate-600">Daftar data buku tematik. Form tambah dan edit dipisah ke halaman khusus.</p>
        </div>
...
const { tematikApi: API_URL, authApi: API_AUTH, stockHistoriesApi: STOCK_HISTORIES_API } = useApiEndpoints()
...
const handleCoverSearch = async (file) => {
  ...
  try {
    ...
    console.error('Error searching cover tematik:', error)
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error('Error fetching data tematik:', error)
...
const openStockHistory = async (item) => {
  ...
  try {
    const url = `${STOCK_HISTORIES_API}?item_type=tematik&item_id=${encodeURIComponent(item.id)}&limit=50`
...
const updateStok = async (id, newStok) => {
  try {
    ...
    console.error('Error updating stok tematik:', error)
