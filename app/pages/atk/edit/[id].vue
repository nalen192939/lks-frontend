  ...
  <div class="min-h-screen bg-slate-100 text-slate-900">
    <header class="border-b border-slate-200 bg-white">
      <div class="mx-auto flex w-full max-w-3xl items-center justify-between gap-3 px-4 py-5 sm:px-6 lg:px-8">
        <div>
          <h1 class="text-2xl font-bold">Edit Data ATK</h1>
          <p class="mt-1 text-sm text-slate-600">Perbarui data ATK sesuai kebutuhan.</p>
        </div>
        <NuxtLink
          to="/atk"
...
const removeImage = () => {
  ...
  const fileInput = document.getElementById('file-upload-atk-edit')
...
const fetchDetail = async () => {
  ...
  try {
    ...
    form.value = {
      ...
      kategori: item.kategori || 'alat_tulis',
      satuan: item.satuan || 'pcs',
      stok: item.stok || 0,
      merk: item.merk || item.keterangan || '',
      jumlah: item.jumlah || 0,
      gambar: null,
      gambarPreview: item.gambar ? getImageUrl(item.gambar) : null
    }
  } catch (error) {
    console.error('Error fetching detail atk:', error)
    alert('Data ATK tidak ditemukan atau gagal dimuat.')
    await router.push('/atk')
  } finally {
...
const updateData = async () => {
  ...
  try {
    ...
    formData.append('kategori', form.value.kategori || 'alat_tulis')
    formData.append('satuan', form.value.satuan || 'pcs')
    ...
    if (!response.ok) {
      ...
      throw new Error(error.message || 'Gagal update data ATK')
    }

    alert('Data ATK berhasil diupdate.')
    await router.push('/atk')
  } catch (error) {
    console.error('Error updating data atk:', error)
    alert(`Gagal mengupdate data ATK: ${error.message}`)
  } finally {
    saving.value = false
  }
}
